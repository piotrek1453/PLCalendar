...
package binary_counter_comp is
  generic (
    COUNTER_LEN : integer := 32
  );

  type binary_counter_in_type is record
    countIn : std_logic;
    resetIn : std_logic;
  end binary_counter_in_type;

  type binary_counter_out_type is record
    countOut    : std_logic_vector(COUNTER_LEN - 1 downto 0);
    overflowOut : std_logic;
  end binary_counter_out_type;
end package;
...
package
  use work.binary_counter_comp.all;

  entity binary_counter is
    port (
      inputs  : in binary_counter_in_type;
      outputs : out binary_counter_out_type;
    );
  end entity;
  ...
  begin
    ...
    counter_fsm_sequential : process (all) begin
      if resetIn = '0' then
    ...
    counter_fsm_combinatorial : process (all)
    begin
